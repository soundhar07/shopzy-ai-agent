...
server instructions + tool descriptions and builds its own payloads to find the best deals.
"""
...
def setup_environment() -> None:
    ...
    os.environ["LANGSMITH_PROJECT"] = "Shopzy Smart Shopping list"
...
async def get_mcp_tools():
    """Return MCP tool list from the persistent SSE server — no subprocess."""
...
def run_mcp_tool(state: MessagesState, config: RunnableConfig, store: BaseStore):
    """Execute MCP tool calls via the persistent SSE server — no subprocess."""
    last_msg = state["messages"][-1]
    results = []

    async def _invoke_all():
        # Reuses the module-level MCP_CLIENT — no new process spawned
